...
public partial class FindMusic : ComponentBase
{
    [Inject]
    ...
    private async Task CreateLike(Singer singer,Album album,Musicc musicc)
    {
        AddLikeSong addLikeSong = new AddLikeSong();

        addLikeSong.SingerID = singer.SingerID;
        addLikeSong.UserID = userGuid;
        addLikeSong.MusicID = musicc.MusicID;
        addLikeSong.AlbumID = album.AlbumID;
        addLikeSong.SongTableName = SongTableName;

        var response = await Http.PostAsJsonAsync("/addLikeSong", addLikeSong);
    }
